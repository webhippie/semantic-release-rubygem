...
const writeVersion = async ({
  ...
  const newContents = versionContents.replace(versionRegex, `$1${gemVersion}$2`);
...
const bundleInstall = async (
  { updateGemfileLock, cwd, env, logger, stdout, stderr }
) => {
  ...
  const installResult = execa.command(
    command,
    { cwd, env }
  );

  installResult.stdout.pipe(
    stdout,
    { end: false }
  );

  installResult.stderr.pipe(
    stderr,
    { end: false }
  );
...
const buildGem = async (
  { gemspec, gemName, version, cwd, env, logger, stdout, stderr }
) => {
  const gemFile = `${gemName}-${version}.gem`;

  logger.log('Building gem `%s`', gemFile);

  const buildResult = execa(
    'gem',
    ['build', gemspec],
    { cwd, env }
  );
...
export default async function prepare(
  ...
  if (gemFileDir) {
    ...
    const gemFileDestination = path.resolve(
      cwd,
      gemFileDir.trim(),
      gemFile
    );
